...
Code block changed by Qingyuan: In the dfNCBI I can see there are some whole mitochondrial genomes and very short sequences. To prevent this situation and make the next filtering step easier you can limit the sequence length from 500 to 800 for COI and from 600 to 1200 for CytB.
```{r, results = "hide"}
...
FetchFastaFiles(searchTerm = "Salmoninae[ORGN] AND COI[Gene] AND 500:800[SLEN]", 
                seqsPerFile = 100, fastaFileName = "Salmon_fetch_1.fasta")
FetchFastaFiles(searchTerm = "Salmoninae[ORGN] AND CytB[Gene] AND 600:1200[SLEN])", 
                seqsPerFile = 100, fastaFileName = "Salmon_fetch_2.fasta")
# Merge all fasta files and store it as a data frame 
dfNCBI <- MergeFastaFiles(filePattern = "Salmon_fetch_.+.fasta.*") %>%
  mutate(Sequence_Length = nchar(Sequence))
```
...
Code block changed by Qingyuan: I changed the way of extracting gene names into the new column because I notice some of the Title have different format that the gene names are not in the parentheses. (?i) means case insensitive.
```{r, warning = FALSE}
dfSalmon <- dfNCBI %>% 
  ...
  ## Comment from Qingyuan: I change the way of extracting gene name into the new column
...
dfTraining <- dfSalmon %>%
  ...
  ## Comment from Qingyuan: I changed the number because the number of sequences changed
  sample_n(777)
```
...
BuildClassifier <- function(a,b) {
  classifier <- randomForest::randomForest(x = dfTraining[,a:b], 
  y = as.factor(dfTraining$Gene_Name), ntree = 50, importance = TRUE)
  return(classifier)
}
